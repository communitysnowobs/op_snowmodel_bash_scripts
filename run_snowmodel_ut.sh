...
#This script will execute the complete assimilative snowmodel run for the wasatch (UT)
...
gdal_translate -q -of GTiff -a_srs EPSG:32612 -a_ullr 432255 4507095 461055 4476735 $fin $fout
#clean up
rm "${fin}"
...
mv $fout $fin
rm -f $fout
gsutil cp $fin gs://cso_test_upload/ut_domain/swed_wo_assim/
...
mv $fout $fin
rm -f $fout
gsutil cp $fin gs://cso_test_upload/ut_domain/swed_wi_assim/
#let's move it to /scratch and get it off of depot
...
mv $fout $fin
rm -f $fout
gsutil cp $fin gs://cso_test_upload/ut_domain/snod_wo_assim/
#let's move it to /scratch and get it off of depot
...
mv $fout $fin
rm -f $fout
gsutil cp $fin gs://cso_test_upload/ut_domain/snod_wi_assim/
#let's move it to /scratch and get it off of depot
