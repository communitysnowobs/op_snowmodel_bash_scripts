...
ipython met_data_backfill_ca.py
conda deactivate
...
rm -f $fin
gsutil cp $fout gs://cso_test_upload/ca_domain/swed_wo_assim/
...
gdal_translate -q $fin $fout -co TILED=YES -co COPY_SRC_OVERVIEWS=YES -co COMPRESS=DEFLATE
rm -f $fin
gsutil cp $fout gs://cso_test_upload/ca_domain/snod_wo_assim/
#let's move it to /scratch and get it off of depot
...
gdal_translate -q $fin $fout -co TILED=YES -co COPY_SRC_OVERVIEWS=YES -co COMPRESS=DEFLATE
rm -f $fin
gsutil cp $fout gs://cso_test_upload/ca_domain/swed_wi_assim/
#let's move it to /scratch and get it off of depot
...
gdal_translate -q $fin $fout -co TILED=YES -co COPY_SRC_OVERVIEWS=YES -co COMPRESS=DEFLATE
rm -f $fin
gsutil cp $fout gs://cso_test_upload/ca_domain/snod_wi_assim/
#let's move it to /scratch and get it off of depot
