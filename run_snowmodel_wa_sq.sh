...
#This script will execute the complete assimilative snowmodel run for the snoq (wa)
...
outpath="/scratch/op_snowmodel_outputs/WA_SQ/"
...
gdal_translate -q -of GTiff -a_srs EPSG:32610 -a_ullr 594765 5267895 637635 5226915 $fin $fout
#clean up
rm "${fin}"
...
mv $fout $fin
rm -f $fout
gsutil cp $fin gs://cso_test_upload/wa_sq_domain/swed_wo_assim/
...
mv $fout $fin
rm -f $fout
gsutil cp $fin gs://cso_test_upload/wa_sq_domain/swed_wi_assim/
#let's move it to /scratch and get it off of depot
...
mv $fout $fin
rm -f $fout
gsutil cp $fin gs://cso_test_upload/wa_sq_domain/snod_wo_assim/
#let's move it to /scratch and get it off of depot
...
mv $fout $fin
rm -f $fout
gsutil cp $fin gs://cso_test_upload/wa_sq_domain/snod_wi_assim/
#let's move it to /scratch and get it off of depot
