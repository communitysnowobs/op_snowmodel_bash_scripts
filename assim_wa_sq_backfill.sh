...
ipython met_data_backfill_wa_sq.py
conda deactivate
...
ipython assim_backfill_wa_sq.py
conda deactivate
...
gdal_translate -q $fin $fout -co TILED=YES -co COPY_SRC_OVERVIEWS=YES -co COMPRESS=DEFLATE
rm -f $fin
gsutil cp $fout gs://cso_test_upload/wa_sq_domain/swed_wi_assim/
#let's move it to /scratch and get it off of depot
...
gdal_translate -q $fin $fout -co TILED=YES -co COPY_SRC_OVERVIEWS=YES -co COMPRESS=DEFLATE
rm -f $fin
gsutil cp $fout gs://cso_test_upload/wa_sq_domain/snod_wi_assim/
#let's move it to /scratch and get it off of depot
